...
final class RemoteServer(
  ...
  override def compile(sbtData: SbtData,
    ...
    val arguments = Arguments(sbtData, compilerData, compilationData, None).asStrings(pathTranslator)
  ...
  override def computeStamps(outputFiles: Seq[Path], analysisFile: Path, client: Client): Either[Server.ServerError, ExitCode] = {
    val arguments = ComputeStampsArguments(outputFiles, analysisFile).asStrings(pathTranslator)
  ...
  private val pathTranslator: PathTranslator = _.toAbsolutePath.normalize().toString
}
