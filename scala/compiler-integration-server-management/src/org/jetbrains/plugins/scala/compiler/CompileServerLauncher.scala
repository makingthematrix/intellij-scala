          ...
          .map { process =>
            ...
            val port = {
              ...
              if (local) CompileServerPort.Local(portReportedByServer)
                ...
                import kotlinx.coroutines.future.FutureKt.asCompletableFuture
                val localPort = asCompletableFuture(EelTunnelsKt.forwardLocalPort(scope, tunnels, portReportedByServer)).get().intValue()
                CompileServerPort.Remote(localPort, portReportedByServer)
              }
            }
