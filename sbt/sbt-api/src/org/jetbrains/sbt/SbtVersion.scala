...
object SbtVersion {
  ...
  object Latest {
    ...
    private val Sbt_1_12 = SbtVersion("1.12.0")

    val Sbt_1: SbtVersion = Sbt_1_12
    val Sbt_2: SbtVersion = SbtVersion("2.0.0-RC8")
  ...
  private def standardBinaryVersion(sbtVersion: Version): Version = {
    ...
    if ((sbtVersion ~= Version("1.0.0")) && sbtVersion.presentation.contains("-M"))
      sbtVersion
    // sbt uses binary version "x" starting from sbt 2
    else if (sbtVersion.major(1) >= Version("2")) {
      val major = sbtVersion.major(1).presentation
      Version(major)
    }
    // sbt uses binary version x.0 for [1.0.x,2.x.y)
